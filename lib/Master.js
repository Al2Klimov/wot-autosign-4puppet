...
const {middleware: {handleErrors}, Promisify} = require("./util");
            ...
            if (this.server === null) {
                let [hostcert, hostprivkey, cacert, cacrl] = await Promise.all(
                    ["hostcert", "hostprivkey", "cacert", "cacrl"].map(
                        key => Promisify(readFile, this.puppetConfig.get(key), "utf8")
                    )
                );
                ...
                try {
                    await Promise.all(this.config.listen.map(
                        ep => Promisify(listen, this.server, ep.port, ep.address)
                    ));
                } catch (e) {
