        ...
        return express()
            ...
            .put(
                ...
                (req, res, next) => {
                    ...
                    let responsible = this.config.web_of_trust.filter(wot => wot.trustee(cn));

                    if (responsible.length) {
                        res.locals.wot_autosign_4puppet = {responsible: responsible};
                        next();
                    } else {
                        res.status(403).end();
                    }
                },
                util.middleware.handleErrors(jsonParser, (err, req, res) => {
                    res.status(400).end();
                }),
                (req, res) => {
