...
const {fs: {readFile}, https: {Server: {close: httpsServerClose}}} = require("./promisified");
...
const {middleware: {handleErrors}} = require("./util");
        ...
        return this.stateChangeMutex.enqueue(async () => {
            if (this.server === null) {
                ...
                try {
                    ...
                    await Promise.all(this.config.listen.map(ep => listenEP(ep.port, ep.address)));
                } catch (e) {
