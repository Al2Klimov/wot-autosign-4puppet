            ...
            if (this.server === null) {
                let [hostcert, hostprivkey, cacert, cacrl] = await Promise.all(
                    ["hostcert", "hostprivkey", "cacert", "cacrl"].map(
                        key => util.Promisify(fs.readFile, this.puppetConfig.get(key), "utf8")
                    )
                );
                ...
                try {
                    await Promise.all(this.config.listen.map(
                        listen => util.Promisify(sc.net.Server.listen, this.server, listen.port, listen.address)
                    ));
                } catch (e) {
                    await util.Promisify(cb => this.server.close(cb));
                    this.server = null;
                    throw e;
                }
            }
