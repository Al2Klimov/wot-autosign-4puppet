...
async function collectReadings(paths, dest, level) {
    await Promise.all(paths.map(path => (async () => {
        let stats = await util.Promisify(fs.stat, path);

        if (stats.isDirectory()) {
            await collectReadings(
                (await util.Promisify(fs.readdir, path)).map(subPath => fspath.join(path, subPath)),
                dest,
                level + 1
            );
            ...
            dest.push({
                path: path,
                reading: util.Promisify(fs.readFile, path, "utf8"),
                content: undefined,
                parsed: undefined
            });
        }
